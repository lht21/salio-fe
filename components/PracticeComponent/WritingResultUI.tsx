...
export default function WritingResultUI({ data, onHomePress, onRetryPress }: WritingResultUIProps) {
  ...
  const getIconForTitle = (title: string) => {
    if (title.includes('Nội dung')) return <TextAUnderlineIcon size={35} color={colors.textBrand} weight="fill" />;
    if (title.includes('Từ vựng')) return <TranslateIcon size={35} color={colors.textBrand} weight="fill" />;
    if (title.includes('Ngữ pháp')) return <BookOpenIcon size={35} color={colors.textBrand} weight="fill" />;
    if (title.includes('mạch lạc')) return <LinkIcon size={35} color={colors.textBrand} weight="fill" />;
    return <TextTIcon size={35} color={colors.textBrand} weight="fill" />;
  };
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
        ...
        <View style={styles.headerTop}>
          ...
          <View style={styles.headerActions}>
            <TouchableOpacity style={styles.helpButton} onPress={() => setShowActionMenu(true)}>
              <SealQuestionIcon size={20} color={colors.background} weight="bold" />
            </TouchableOpacity>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
        {activeTab === 'overview' ? (
          <>
            <LinearGradient colors={[colors.primary, colors.primaryLight]} style={styles.scoreBanner}>
              ...
              <View style={styles.statsRow}>
                <StatCircle icon={<TextTIcon size={20} color={colors.textBrand} />} value={`${charCount}/700`} label="ký tự" />
                <StatCircle icon={<ClockIcon size={20} color={colors.textBrand} />} value={formatTime(timeUsed)} label="phút" />
                <StatCircle icon={<TargetIcon size={20} color={colors.textBrand} />} value={`${Math.round((aiScore / 50) * 100)}%`} label="hoàn thành" />
              </View>
            </LinearGradient>
      ...
      <View style={styles.footer}>
        <TouchableOpacity style={styles.retryButton} activeOpacity={0.8} onPress={onHomePress}>
          <HouseIcon size={20} color={colors.textBrand} weight="duotone" />
        </TouchableOpacity>
...
const getStyles = (colors: any) => StyleSheet.create({
  ...
  header: { paddingHorizontal: Padding.padding_15, paddingTop: Padding.padding_10, backgroundColor: colors.background },
  headerTop: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: Gap.gap_15 },
  headerTitle: { fontFamily: FontFamily.lexendDecaSemiBold, fontSize: FontSize.fs_20, color: colors.textPrimary },
  headerActions: { flexDirection: 'row', alignItems: 'center', gap: Gap.gap_10 },
  helpButton: { width: 32, height: 32, borderRadius: 16, backgroundColor: colors.textBrand, justifyContent: 'center', alignItems: 'center' },
  ...
  mainScoreCircle: {
    width: 120, height: 120, borderRadius: 60, backgroundColor: colors.textBrand, justifyContent: 'center', alignItems: 'center',
    shadowColor: colors.textBrand, shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.1, shadowRadius: 10, elevation: 5
  },
  mainScoreText: { fontFamily: FontFamily.lexendDecaSemiBold, fontSize: 40, color: colors.vang, lineHeight: 45 },
  maxScoreText: { fontFamily: FontFamily.lexendDecaMedium, fontSize: FontSize.fs_14, color: colors.background },
  ...
  footer: {
    ...
    paddingBottom: Padding.padding_30, backgroundColor: colors.background, borderTopWidth: 1, borderTopColor: colors.borderDefault, gap: Gap.gap_15
  },
  retryButton: {
    paddingHorizontal: Padding.padding_15, flexDirection: 'row', alignItems: 'center', justifyContent: 'center',
    height: 47, borderRadius: 37, borderWidth: 1.5, borderColor: colors.borderDefault, backgroundColor: colors.background, gap: Gap.gap_8
  },
